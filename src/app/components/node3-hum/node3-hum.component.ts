...
@Component({
  selector: 'app-node3-hum',
  templateUrl: './node3-hum.component.html',
  styleUrls: ['./node3-hum.component.css']
})
export class Node3HumComponent implements OnInit {
  ...
  onConnected() {
    ...
    this.toast.success('Humidity Sensor 3 is now online');
    this.client.subscribe("/agrilab/node3/humidity");
  }

  // // Initialiaze Paho message and Send a message
  sendMessage(message: string) {
    let packet = new Paho.MQTT.Message(message);
    packet.destinationName = "/agrilab/node3/humidity";
  ...
  onConnectionLost() {
    ...
    this.toast.warning('Humidity Sensor 3 is now offline')
    };
  }
}
