...
@Component({
  selector: 'app-node3-temp',
  templateUrl: './node3-temp.component.html',
  styleUrls: ['./node3-temp.component.css']
})
export class Node3TempComponent implements OnInit {
  ...
  ngOnInit() {
   ...
   onConnected() {
    this.toast.success('Temperature Sensor 3 is now online');
    this.client.subscribe("/agrilab/node3/temperature");
  }

  // Initialiaze Paho message and Send a message
  sendMessage(message: string) {
    let packet = new Paho.MQTT.Message(message);
    packet.destinationName = "/agrilab/node3/temperature";
  ...
  onConnectionLost() {
    this.client.onConnectionLost = (responseObject: Object) => {
      ...
      this.toast.warning('Temperature Sensor 3 is now offline')
    };
  }
