...
@Component({
  selector: 'app-node5-temp',
  templateUrl: './node5-temp.component.html',
  styleUrls: ['./node5-temp.component.css']
})
export class Node5TempComponent implements OnInit {
  ...
  ngOnInit() {
   ...
   onConnected() {
    this.toast.success('Temperature Sensor 5 is now online');
    this.client.subscribe("/agrilab/node5/temperature");
  }

  // Initialiaze Paho message and Send a message
  sendMessage(message: string) {
    let packet = new Paho.MQTT.Message(message);
    packet.destinationName = "/agrilab/node5/temperature";
  ...
  onConnectionLost() {
    this.client.onConnectionLost = (responseObject: Object) => {
      ...
      this.toast.warning('Temperature Sensor 5 is now offline')
    };
  }
