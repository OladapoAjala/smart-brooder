...
@Component({
  selector: 'app-node2-temp',
  templateUrl: './node2-temp.component.html',
  styleUrls: ['./node2-temp.component.css']
})
export class Node2TempComponent implements OnInit {
  ...
  ngOnInit() {
   ...
   onConnected() {
    this.toast.success('Temperature Sensor 2 is now online');
    this.client.subscribe("/agrilab/node2/temperature");
  }

  // Initialiaze Paho message and Send a message
  sendMessage(message: string) {
    let packet = new Paho.MQTT.Message(message);
    packet.destinationName = "/agrilab/node2/temperature";
  ...
  onConnectionLost() {
    this.client.onConnectionLost = (responseObject: Object) => {
      ...
      this.toast.warning('Temperature Sensor 2 is now offline')
    };
  }
