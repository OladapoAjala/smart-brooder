...
@Component({
  selector: 'app-node2-hum',
  templateUrl: './node2-hum.component.html',
  styleUrls: ['./node2-hum.component.css']
})
export class Node2HumComponent implements OnInit {
  ...
  onConnected() {
    ...
    this.toast.success('Humidity Sensor 2 is now online');
    this.client.subscribe("/agrilab/node2/humidity");
  }

  // // Initialiaze Paho message and Send a message
  sendMessage(message: string) {
    let packet = new Paho.MQTT.Message(message);
    packet.destinationName = "/agrilab/node2/humidity";
  ...
  onConnectionLost() {
    ...
    this.toast.warning('Humidity Sensor 2 is now offline')
    };
  }
