...
@Component({
  selector: 'app-node4-hum',
  templateUrl: './node4-hum.component.html',
  styleUrls: ['./node4-hum.component.css']
})
export class Node4HumComponent implements OnInit {
  ...
  onConnected() {
    ...
    this.toast.success('Humidity Sensor 4 is now online');
    this.client.subscribe("/agrilab/node4/humidity");
  }

  // // Initialiaze Paho message and Send a message
  sendMessage(message: string) {
    let packet = new Paho.MQTT.Message(message);
    packet.destinationName = "/agrilab/node4/humidity";
  ...
  onConnectionLost() {
    ...
    this.toast.warning('Humidity Sensor 4 is now offline')
    };
  }
}
