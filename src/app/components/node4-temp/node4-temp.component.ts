...
@Component({
  selector: 'app-node4-temp',
  templateUrl: './node4-temp.component.html',
  styleUrls: ['./node4-temp.component.css']
})
export class Node4TempComponent implements OnInit {
  ...
  ngOnInit() {
   ...
   onConnected() {
    this.toast.success('Temperature Sensor 4 is now online');
    this.client.subscribe("/agrilab/node4/temperature");
  }

  // Initialiaze Paho message and Send a message
  sendMessage(message: string) {
    let packet = new Paho.MQTT.Message(message);
    packet.destinationName = "/agrilab/node4/temperature";
  ...
  onConnectionLost() {
    this.client.onConnectionLost = (responseObject: Object) => {
      ...
      this.toast.warning('Temperature Sensor 4 is now offline')
    };
  }
