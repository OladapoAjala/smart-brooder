...
@Component({
  selector: 'app-node5-hum',
  templateUrl: './node5-hum.component.html',
  styleUrls: ['./node5-hum.component.css']
})
export class Node5HumComponent implements OnInit {
  ...
  onConnected() {
    ...
    this.toast.success('Humidity Sensor 5 is now online');
    this.client.subscribe("/agrilab/node5/humidity");
  }

  // // Initialiaze Paho message and Send a message
  sendMessage(message: string) {
    let packet = new Paho.MQTT.Message(message);
    packet.destinationName = "/agrilab/node5/humidity";
  ...
  onConnectionLost() {
    ...
    this.toast.warning('Humidity Sensor 5 is now offline')
    };
  }
}
